...
while True:
    for ip in ip_list:
        try:
            r = requests.get('http://' + ip + ':5679/model_download', timeout=5)
            l = len(r.content)
            if ip + str(l) in ip_store:
                print 'pass:', ip
                continue
            if l != 403247:
                with open(ip + '-' + time.strftime("%Y%m%d_%H%M%S", time.localtime()) + '-model_save.zip', 'wb') as f:
                    f.write(r.content)
                print(ip, str(l))
                ip_store.append(ip + str(l))
            else:
                print 'pass:', ip
        except Exception, e:
